...
async function request(method: 'GET' | 'POST' | 'PATCH' | 'DELETE', name?: string, id?: string, signal?: AbortSignal, action?: 'move-to-tasks'): Promise<unknown> {
  ...
  try {
    response = await fetch(url, {
      ...
      body: method === 'GET' || method === 'DELETE' || action ? undefined : JSON.stringify({ name }),
