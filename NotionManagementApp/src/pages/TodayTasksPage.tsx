        ...
        {tasks.length === 0 && !error ? <div className="empty-state"><span>✓</span><h3>Nothing planned</h3><p>Your Notion today view is empty.</p></div> : (
          <ul>{tasks.map(task => (
            <li className="today-item" key={task.id}>
              <div className="today-details"><strong>{task.name}</strong><small>{task.projects.length ? task.projects.join(', ') : 'No project'}</small></div>
              <div className="today-actions">
                <button className={`status-button notion-${colorOf(task.status, statuses)}`} disabled={busy === task.id} onClick={() => setMenu(menu === task.id ? null : task.id)}>
              ...
              {menu === task.id && <div className="status-options" role="menu">
