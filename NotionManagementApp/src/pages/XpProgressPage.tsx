...
export function XpProgressPage() {
  ...
  useEffect(() => { void load(period, undefined); }, [period]);
  ...
  return <>
    ...
    <section className="xp-progress-card" aria-busy={loading}>
      ...
      <div className="section-heading"><h2>{progress ? periodLabel(progress) : labels[period]}</h2><button className="text-button" type="button" onClick={() => void load(period, periodStart)} disabled={loading}>Odśwież</button></div>
